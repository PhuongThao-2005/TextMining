...
def build_report(
    *,
    ...
    lines: list[str] = [
        ...
        "",
        f"This report analyzes `{len(runs)}` saved runs under `{root}`. The primary paired inference is GPT-4o-mini Base versus CoT; other model pairs remain descriptive, with available data-quality checks exposed below. Paired bootstrap intervals use {bootstrap_reps:,} replicates (seed {seed}).",
        "",
