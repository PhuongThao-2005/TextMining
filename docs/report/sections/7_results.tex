...
retrieval result, while BGE has the strongest lexical answer overlap. By
contrast, the tested hybrid and graph configurations reduce exact gold-chunk
coverage. These statements concern a finite benchmark and its gold evidence,
not legal advice quality in deployment.
...
to 0.4257 and takes 27.855 seconds when amortized BM25 time is included. Graph
contexts appear more identity-precise because they contain only 5.605--6.305
chunks on average, but their reduced context size also removes gold coverage.
...
prefer the chunk-only representation. Do not deploy the current BM25+RRF or
one-hop graph setup as the default retrieval path without retuning. Prefer
Jina when retrieval quality justifies its 1.278-second reranking p50, or
mMiniLM when its lower 0.385-second cost is more important. Retain the Base
prompt for lexical answer overlap. In all cases, present citations as sources
to inspect and retain human review for legal interpretation.
